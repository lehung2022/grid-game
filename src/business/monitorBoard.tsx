...
const monitorBoard = (board: BoardElement[]): boolean => {
  ...
  for (let i = 0; i < board.length; i++) {
    ...
    if (
      i - (width - 1) >= 0 &&
      i - (width - 2) >= 0 &&
      board[i].color === board[i - (width - 1)].color &&
      board[i].color === board[i - (width - 2)].color
    )
      return true;
  ...
  for (let i = 0; i < 48; i++) {
    ...
    if (
      ...
      i + width * 3 < 64 &&
      board[i].color === board[i + width].color &&
      board[i].color === board[i + width * 3].color
    )
      return true;
    ...
    if (
      i + (width - 1) < 64 &&
      i + (width * 2 - 1) < 64 &&
      atVerticalRange_column - 1 >= 0 &&
      board[i].color === board[i + (width - 1)].color &&
      board[i].color === board[i + (width * 2 - 1)].color
    )
      return true;
